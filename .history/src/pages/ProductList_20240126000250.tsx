...
const ProductList: React.FC<ProductListProps> = ({ products, onCreate, onDelete, onUpdate }) => {
  ...
  return (
    <div className="container mx-auto my-8 p-4 bg-white shadow-lg rounded-lg block gap-64 text-center ml-32">
      <h2 className="text-3xl font-bold mb-4 text-left">Product List</h2>
      <table className="w-full border border-cyan-700 w-max-80 shadow-2xl box-shadow: 0 4px 6px -1px rgba(169, 169, 169, 0.1), 0 2px 4px -1px rgba(169, 169, 169, 0.06);">
        <thead>
        ...
        <tbody>
          {products.map((product) => (
            <tr key={product.id} className="border-b">
              <td>
                <img src={product.photo} alt={product.name} className="w-48 h-36 object-cover" />
              </td>
