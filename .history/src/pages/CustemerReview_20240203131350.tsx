...
const ReviewPage: React.FC = () => {
  const initialReviews: Review[] = [
    ...
    { id: 2, name: 'Jane Smith', comment: 'Not bad, but could be better.', rating: 3 },
    { id: 3, name: 'Alice Johnson', comment: 'Excellent service!', rating: 4 },
    { id: 4, name: 'Bob Williams', comment: 'Product was damaged upon arrival.', rating: 2 },
    { id: 5, name: 'Eva Davis', comment: 'Fast shipping, good quality.', rating: 5 },
  ];

  const [reviews, setReviews] = useState<Review[]>(initialReviews);
  const [modalOpen, setModalOpen] = useState<boolean>(false);
  const [newReview, setNewReview] = useState<Review>({ id: 0, name: '', comment: '', rating: 0 });

  const handleAddReview = () => {
    setReviews([...reviews, newReview]);
    setModalOpen(false);
    setNewReview({ id: 0, name: '', comment: '', rating: 0 });
  };
